...
test("トリガーを押すとメニューが開く", async () => {
  ...
  await expect.element(view.getByRole("menu")).toBeInTheDocument();
});
...
test("ブックマークボタンが表示される", async () => {
  ...
  await expect.element(view.getByRole("menuitem", { name: "Save" })).toBeInTheDocument();
});
...
test("ブックマークボタンを押すと、ブックマークから削除される", async () => {
  ...
  await view.getByRole("menuitem", { name: "Saved" }).click();
