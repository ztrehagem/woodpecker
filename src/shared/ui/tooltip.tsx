    ...
    <Lib.Root>
      ...
      <Lib.Portal className="relative z-50">
        <Lib.Positioner side={side} sideOffset={8}>
          <Lib.Popup className="bg-highlight rounded-md px-3 py-1.5">
            <Lib.Arrow className="before:bg-highlight relative block h-1.5 w-3 overflow-clip before:absolute before:bottom-0 before:left-1/2 before:h-[calc(6px*sqrt(2))] before:w-[calc(6px*sqrt(2))] before:transform-[translate(-50%,50%)_rotate(45deg)] before:content-[''] data-[side=bottom]:-top-1.5 data-[side=left]:-right-2.25 data-[side=left]:rotate-90 data-[side=right]:-left-2.25 data-[side=right]:-rotate-90 data-[side=top]:-bottom-1.5 data-[side=top]:rotate-180" />
            {tooltip}
          </Lib.Popup>
        </Lib.Positioner>
      </Lib.Portal>
